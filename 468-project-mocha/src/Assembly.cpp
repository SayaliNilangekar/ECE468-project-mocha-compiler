	...
	for(std::vector<std::string>::iterator it=IRcodeList.begin();it!=IRcodeList.end();++it){ 
		...
		if(std::find(AssemblyStrVector.begin(), AssemblyStrVector.end(),"\n") != AssemblyStrVector.end()) {
			...
			std::vector<std::string>::iterator bound;
  			bound = std::stable_partition (AssemblyStrVector.begin(), AssemblyStrVector.end(), [](std::string check) {return check == "\n"; });
			for (std::vector<std::string>::iterator it = AssemblyStrVector.begin(); it!=bound; ++it){
				moveVector.push_back(*it); 
			}
			for (std::vector<std::string>::iterator it = bound; it!=AssemblyStrVector.end(); ++it){
				operVector.push_back(*it); 
			}
		
			string AssemStr1;
			string AssemStr2;

			len1 = moveVector.size();
			len2 = operVector.size();
					
			for (int k = 0; k<len1; k++){
				AssemStr1 = AssemStr1 + moveVector[k];
			} 
			
			for (int k = 0; k<len2; k++){
				AssemStr2 = AssemStr2 + operVector[k];
			} 

			returnVector.push_back(AssemStr1);			
			returnVector.push_back(AssemStr1);			
			...
			returnVector.push_back(AssemStr);			
		}
...
//Instead of creating vectors for assembly, simply std::cout a concatenated string.
std::vector IRtoAssembly(vector IR){
